// debugMagnets.js — Análise brutal de magnets para detectar inconsistências
// Uso: node debugMagnets.js [arquivo.json]
// O arquivo JSON deve conter um array de objetos { magnet, originalTitle?, provider? }

import { analisarMagnet } from './src/magnet/magnetHelper.js'; // ajuste o caminho se necessário
import fs from 'fs';

function extractDnFromMagnet(magnet) {
...
async function analyzeMagnet(item) {
...
main().catch(err => console.error(err));// debugMagnets.mjs — Análise brutal de magnets para detectar inconsistências
// Uso: node scripts/debugMagnets.mjs <arquivo.json>
