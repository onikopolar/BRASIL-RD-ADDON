...
function extrairDn(magnet?: string | null): string | null {
  if (!magnet) return null;
  // pega tudo depois de "dn=" até o próximo "&" (ou fim)
...
async function main() {
  ...
  for (const [id, ts] of grupos) {
    ...
    for (const t of ts) {
      ...
      const dn = extrairDn(t.magnet) ?? '(sem dn)';
      console.log(`  [${prov}] ${temp}${ep}  ${dn}`);
    }
    console.log('');
  }
