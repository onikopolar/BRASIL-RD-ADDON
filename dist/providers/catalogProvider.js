...
class CatalogProvider {
    ...
    async performIntelligentScraping(request, season, episode) {
        ...
        try {
            ...
            const filteredTorrents = await this.filterAndValidateTorrents(deduplicatedTorrents, imdbId, request, finalSeason, finalEpisode, tmdbSearchData?.imdbTitles || null);
            ...
            const hasCompletePack = filteredTorrents.valid.some(torrent => torrent.title.toLowerCase().includes('temporada') &&
    ...
    async saveValidTorrentsToCatalog(validTorrents, request, season, episode, imdbTitles = null, hasCompletePack = false) {
        const imdbId = this.extractBaseImdbId(request.imdbId || request.id);
        ...
        for (const torrent of validTorrents) {
            try {
                ...
                const result = await this.autoMagnetService.autoAddMagnet(torrent.magnet, torrent.title, imdbId, request.type, torrent.seeders, torrent.quality, torrent.size, season, episodeValue);
