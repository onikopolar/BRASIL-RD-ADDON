...
class AutoMagnetService {
    ...
    async autoAddMagnet(magnetLink, torrentTitle, imdbId, type, seeds = 50, quality, size, imdbSeason, imdbEpisode) {
        ...
        try {
            ...
            const titleMatchResult = await this.validateTitleWithCache(torrentTitle, imdbId, imdbSeason, imdbEpisode !== null ? imdbEpisode : undefined);
    ...
    async validateTitleWithCache(torrentTitle, imdbId, season, episode) {
    ...
    async saveToDatabaseOptimized(magnetData, imdbTitles, allQualities = [], titleMatchResult) {
        try {
    ...
    async extrairMagnetHash(magnet) {
        const dados = await (0, magnetHelper_js_1.analisarMagnet)(magnet);
    ...
    async testTitleValidation(torrentTitle, imdbId, testSeason, testEpisode) {
        try {
            ...
            const multiplos = episodeMatcher.temMultiplosEpisodios(torrentTitle);
            const ehPack = episodeMatcher.ehPackTemporadaCompleta(torrentTitle);
            const matchResult = await titleFilter.titulosCombinam(torrentTitle, imdbId, testSeason, testEpisode !== null ? testEpisode : undefined);
