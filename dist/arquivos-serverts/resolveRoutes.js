    ...
    try {
        ...
        if (torrentInfo.torrentId) {
            ...
            return {
                ...
                message: (0, statusHelpers_1.getStatusMessage)(torrentDetails.download_state, Math.round(torrentDetails.progress * 100)),
...
const setupResolveRoutes = (app) => {
    app.get('/resolve/torbox/:apiKey/:infoHash/null/:fileIndex/:filename', async (req, res) => {
        const resolveLogger = new logger_1.Logger('🔄RESOLVE');
        ...
        try {
            ...
            if (tbResult.success) {
                ...
                if (progressStatuses.some(s => statusLower.includes(s))) {
                    ...
                    const response = staticResponseService.getResponseForTorboxStatus(tbResult.status) || StaticResponseService_1.StaticResponse.DOWNLOADING;
    ...
    app.get('/resolve/:magnet/status', async (req, res) => {
        try {
            ...
            return res.json({ success: true, status: info.download_state, progress: Math.round(info.progress * 100), downloaded: ready, message: (0, statusHelpers_1.getStatusMessage)(info.download_state, Math.round(info.progress * 100)), torrentId: existing.id });
        }
        catch (error) {
