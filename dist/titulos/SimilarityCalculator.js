...
class SimilarityCalculator {
    ...
    constructor(_titleCleaner, useTmdbScraper = true) {
        this.RESULT_TTL = 5 * 60 * 1000;
        this.TOKEN_TTL = 30 * 60 * 1000;
        this.FUZZY_TTL = 30 * 60 * 1000;
        this.TMDB_TTL = 10 * 60 * 1000;
        this.EXTRAS_LEGITIMOS = /^(1080p|720p|480p|2160p|1440p|4k|x264|x265|h264|h265|hevc|dual|dublado|legendado|bluray|webrip|webdl|brrip|hdrip|dvdrip|remux)$/;
        this.REGEX_ANO = /^(19|20)\d{2}$/;
        ...
        this.resultCache = new CacheService_js_1.CacheService({ name: 'sim:result' });
        this.tokenCache = new CacheService_js_1.CacheService({ name: 'sim:token' });
        this.fuzzyCache = new CacheService_js_1.CacheService({ name: 'sim:fuzzy' });
        this.tmdbLocalCache = new CacheService_js_1.CacheService({ name: 'sim:tmdb' });
    }
    compararComTitulos(titulosTmdb, tituloCandidato, threshold = SimilarityCalculator.THRESHOLD_PRE_FILTRO) {
    ...
    pontuarTokens(tmdbTokens, candidatoTokens) {
        return this.calcularScore(tmdbTokens, candidatoTokens);
    }
    async smartTitleContainsCheck(torrentTitle, imdbId, _torrentMetadata, rawTitleForLanguage, preFetchedTmdbData) {
        const inicio = Date.now();
        const season = _torrentMetadata?.season ?? 0;
        const resultKey = `res|${imdbId}|${season}|${(0, TechnicalWords_js_1.normalizarTexto)(torrentTitle)}`;
        ...
        const isCollection = (0, TechnicalWords_js_1.isCollectionTitle)(torrentTitle);
        ...
        if (!isCollection && !dentroDoRangeAnos && anoTorrent !== null && movieInfo.year !== undefined) {
            ...
            if (diverge) {
                ...
                this.resultCache.set(resultKey, result, this.RESULT_TTL);
                this.logger.debug(`GATE_ANO rejeitado | "${torrentTitle.substring(0, 50)}" | ` +
        ...
        const seqTorrent = (0, TechnicalWords_js_1.getPotentialSequelNumbers)(torrentTitle);
        const tmdbDeclaraMesmoNumero = seqTorrent.length > 0
            && titulosValidos.some(t => (0, TechnicalWords_js_1.getPotentialSequelNumbers)(t).some(n => seqTorrent.includes(n)));
        let melhor = null;
        let melhorTitulo = '';
        let seqBloqueou = false;
        for (const titulo of titulosValidos) {
            const { tokens: tmdbTokens } = this.tokensAnoCache(titulo);
            if (tmdbTokens.length === 0)
                continue;
            const seqTmdb = (0, TechnicalWords_js_1.getPotentialSequelNumbers)(titulo);
            ...
            if (seqTorrent.length > 0) {
                if (seqTmdb.length > 0) {
                    ...
                    torrentTokensParaComparar = torrentTokens.filter(t => !/^\d{1,2}$/.test(t) || !seqTorrent.includes(parseInt(t)));
                }
            }
    ...
    async obterMovieInfo(imdbId, season, preFetched) {
        if (preFetched) {
            return {
        ...
        const cachedTmdb = this.tmdbLocalCache.get(key);
        if (cachedTmdb) {
            return {
    ...
    calcularScore(tmdbTokens, torrentTokens) {
        const parteTmdb = (0, TechnicalWords_js_1.extrairNumeroParte)(tmdbTokens.join(' '));
        const parteTorrent = (0, TechnicalWords_js_1.extrairNumeroParte)(torrentTokens.join(' '));
        if (parteTmdb !== null && parteTorrent !== null && parteTmdb !== parteTorrent) {
            return this.montarDetalhes('N5', `Parte divergente: ${parteTorrent} vs ${parteTmdb}`, 0, tmdbTokens, torrentTokens, [], []);
        }
    ...
    matchCurtoSuspeito(tmdbTokens, torrentTokens) {
        if (tmdbTokens.length !== 1)
            return false;
        const tokenTmdb = tmdbTokens[0];
        if (tokenTmdb.length > 3)
            return false;
        if (torrentTokens.length < 3)
            return false;
    ...
    montarDetalhes(nivel, descricao, score, tmdbTokens, torrentTokens, matchedSet, _unmatchedSet) {
    ...
    contemConsecutivo(haystack, needle) {
    ...
    contemSubsequencia(haystack, needle) {
        if (needle.length === 0)
            return false;
        let j = 0;
    ...
    calcularF1(tmdbTokens, torrentTokens) {
        const matchedTmdb = [];
        const matchedTorrent = [];
        const unmatchedTmdb = [];
        const unmatchedTorrent = [];
        const posicoes = [];
        const torrentUsados = new Set();
        for (const tmdbTok of tmdbTokens) {
            let melhorRatio = 0;
            let melhorIdx = -1;
            for (let i = 0; i < torrentTokens.length; i++) {
                if (torrentUsados.has(i))
                    continue;
        ...
        for (let i = 0; i < torrentTokens.length; i++) {
            if (!torrentUsados.has(i))
                unmatchedTorrent.push(torrentTokens[i]);
        }
    ...
    extrairTokensAno(texto) {
        const todos = (0, TechnicalWords_js_1.normalizarTexto)(texto).split(' ').filter(w => w.length > 0);
        const semAno = [];
        const anos = [];
    ...
    imdbDentroDoRangeAnos(years, anoImdb) {
        if (!years || years.length === 0 || anoImdb === undefined)
            return false;
        if (years.length > 1) {
    ...
    logarResultado(torrentTitle, tmdbTitle, detalhes, matches, motivo, tempoMs) {
        const icone = matches ? '✅' : '❌';
...
SimilarityCalculator.THRESHOLD_PRE_FILTRO = 0.5;
SimilarityCalculator.THRESHOLD_FINAL = 0.80;
